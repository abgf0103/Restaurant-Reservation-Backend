...
public class MemberController {
    ...
    public ResponseEntity<?> deleteUser(@CurrentUser CustomUserDetails curruntUser){
        Long userId = curruntUser.getId();

        boolean isDeleted = userService.deleteUser(userId);
        if (isDeleted) {
            return ResponseEntity.ok(new ApiResponse(true , "사용자가 삭제되었습니다",null , null));
        } else {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new ApiResponse(false,"사용자 삭제에 실패했습니다"));
        }
